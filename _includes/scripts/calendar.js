...
function getEvents(sel) {
    return gcal.events(
        new Date(dateString(sel)),
        new Date(dateString(advanceMonth(sel)))
    );
}

function loadEvents() {
    return getEvents(selection)
        .then(events => (events.length ? ELEMENTS.calendarList.innerHTML = "" : ELEMENTS.calendarList.innerHTML = "<p>Nothing this month...</p>", events))
...
function makeEventDOM(event) {
    let dom = h("section", { "aria-label": event.summary, class: Date.parse(event.end.dateTime) > Date.now() ? "future-event" : "past-event" }, [
        h("header", { "aria-haspopup": true }, [
            h("p", { class: "title" }, [event.summary]),
            h("p", { class: "date" }, [new Date(event.start.dateTime).toLocaleDateString()]),
            h("i", { class: "material-icons" }, ["expand_more"]),
        ]),
        h("div", { class: "calendar-body" }, [
            h("div", { class: "calendar-meta" }, [
                h("div", { class: "calendar-time" }, [formatTime(event.start.dateTime, event.end.dateTime)]),
                h("div", { class: "calendar-location" }, [
                    h("a", { href: "https://www.google.com/maps/dir/?api=1&origin=" + encodeURIComponent(event.location), target: "_blank", rel: "noopener" }, [event.location.split(",")[0]])
                ]),
            ]),
...
window.addEventListener("load", function () {
    ...
    ELEMENTS.calendarForward = document.querySelector("nav[for=calendar] a.forward");
